...
function requestWithCallback(url, args, callback) {
  ...
  if (args.stream) {
    ...
    // when stream already consumed, req's `finish` event is emitted and pump will ignore error after finish
    // but if server response timeout later, we will abort the request and emit an error in req
    // so we must manually listen to req's `error` event here to ensure this error is handled
