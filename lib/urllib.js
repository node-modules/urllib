...
function requestWithCallback(url, args, callback) {
  ...
  if (args.headers) {
    // only allow enumerable, ownProperty and no-null value of args.headers
    for (var [name, value] of Object.entries(args.headers)) {
      if (isNull(value)) {
        continue;
      }
  ...
  if (args.files) {
    ...
    for (var [name, value] of Object.entries(formHeaders)) {
      // skip null value headers
      if (isNull(value)) {
        continue;
      }
