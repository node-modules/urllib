...
exports.request = function (url, args, callback) {
  ...
  var req = httplib.request(options, function (res) {
    ...
    if (writeStream) {
      ...
      // it emits a 'finish' event.
      //
      // That means that we should not apply callback until the 'finish' of the
      ...
      return res.pipe(writeStream).on('finish', done.bind(null, null, null, res));
    }
